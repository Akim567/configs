...
-- Fillchars
vim.opt.fillchars = { -- Настройка символов, используемых для заполнения интерфейса
    vert = '|',        -- Вертикальная линия между сплитами (окнами).
    fold = ' ',        -- Символ для закрытых складок (fold).
    eob = ' ',         -- Убирает символы `~`, которые отображаются после конца файла (EndOfBuffer).
    -- diff = '⣿',    -- Символы для диффов (можно заменить на другой, например '|', '-', '/').
    msgsep = '‾',      -- Разделитель сообщений (например, между командной строкой и текстом).
    foldopen = 'v',    -- Символ для открытой складки.
    foldsep = '|',     -- Разделитель между складками.
    foldclose = '>'    -- Символ для закрытой складки.
}
